...
for network_number in range(3):
    ...
    tmp = []
    for data_number in [0,3,4]:
        df_t = pd.read_csv(RESULT_FOLDER+f"csv/{data[data_number]}_network_{l_n}.csv")
        tmp.append(df_t['UN'].values)
    means.append(np.mean(tmp, axis=0))
    stds.append(np.std(tmp, axis=0))

df = pd.DataFrame({'MUJ':df_t['MUJ'].values, 'Metoda':df_t['Metoda'].values,
                   'Mean1':means[0], 'std1':stds[0],
                   'Mean2':means[1], 'std2':stds[1],
                   'Mean3':means[2], 'std3':stds[2]})
df.set_index(["MUJ", "Metoda"], inplace=True)

df.to_csv(RESULT_FOLDER+"tab_con/klasyfikacje.csv")
df.to_latex(RESULT_FOLDER+"tab_con/klasyfikacje.txt", float_format="%.3f",
              column_format="|c|c|c|c|c|c|c|c|", longtable=True)
...
df.to_csv(RESULT_FOLDER+"tab_con/regresje.csv")
df.to_latex(RESULT_FOLDER+"tab_con/regresje.txt", float_format="%.3f",
              column_format="|c|c|c|c|c|c|c|c|", longtable=True)